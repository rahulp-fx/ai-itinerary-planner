...
export default function Itinerary() {
  ...
  useEffect(() => {
    const fetchTrip = async () => {
      try {
        const response = await axios.get(`https://ai-itinerary-planner-henna.vercel.app/${id}`);
  ...
  const handleSuggestion = () => {
    const suggestion = prompt("We'd love your feedback! What feature should we add next?");
    if (suggestion) {
  ...
  if (loading) return <div className="loading">✈️ Loading your dream trip...</div>;
  ...
  return (
    <div className="itinerary-container">
      <div className="nav-header">
        ...
        <button onClick={handleSuggestion} className="suggestion-btn">💡 Suggest Feature</button>
      </div>
      
      <header className="trip-header">
        <h1>Trip from {trip.startPoint} to {trip.destination}</h1>
      ...
      <div className="dashboard-grid">
        ...
        <section className="budget-grid-mini">
          <div className="budget-card">🍔 Food: ₹{generatedPlan.budget_breakdown?.food}</div>
          <div className="budget-card">🚕 Travel: ₹{generatedPlan.budget_breakdown?.travel}</div>
          <div className="budget-card">🏨 Stay: ₹{generatedPlan.budget_breakdown?.stay}</div>
          <div className="budget-card">🎭 Activities: ₹{generatedPlan.budget_breakdown?.activities}</div>
        </section>
      </div>
        ...
        <div className="options-grid">
          {generatedPlan.transport_options?.map((item, index) => (
            <div key={index} className="option-card">
              ...
              <button className="book-btn" onClick={() => handleBookClick(item.link)}>Check</button>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="options-grid">
          {generatedPlan.stay_options?.map((item, index) => (
            <div key={index} className="option-card">
              ...
              <button className="book-btn" onClick={() => handleBookClick(item.link)}>View</button>
            </div>
          ))}
        </div>
      </section>
                ...
                {day.activities.map((activity, i) => (
                  <li key={i} className="activity-item">
                    ...
                    <div className="details">
                      ...
                      {activity.cost > 0 && <span className="cost-badge">₹{activity.cost}</span>}
                    </div>
                  </li>
                ))}
              </ul>
